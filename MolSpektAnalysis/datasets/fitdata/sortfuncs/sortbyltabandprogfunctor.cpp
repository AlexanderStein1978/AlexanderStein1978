...
bool SortByLTabAndProgFunctor::operator()(int n, int m)
{
    if (n==-1) return false;
    if (m==-1) return true;
    LineTable* LineTabn = (n < m_NSources ? m_Sources[n] : m_MW->getLineTable(m_Tab->getSource(n), m_Mol));
    LineTable* LineTabm = (m < m_NSources ? m_Sources[m] : m_MW->getLineTable(m_Tab->getSource(n), m_Mol));
