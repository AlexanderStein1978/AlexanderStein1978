...
SortForExtractNewOrChangedWithoutSourceFunctor::SortForExtractNewOrChangedWithoutSourceFunctor(const FitDataCore * const i_Tab, const QString * const i_SourceOffsetNames,
...
bool SortForExtractNewOrChangedWithoutSourceFunctor::operator ()(const int n, const int m)
{
    ...
    const int v1 = m_Tab->get_v(n), v2 = m_Tab->get_v(m);
    if (v1 < v2) return true;
    if (v1 > v2) return false;
    const int J1 = m_Tab->getJ(n), J2 = m_Tab->getJ(m);
    ...
    const double E1 = m_Tab->getEnergy(n) - DeltaE1, E2 = m_Tab->getEnergy(m) - DeltaE2;
