...
ControlWindow::ControlWindow() : window(NULL)
{
    ...
    L->addWidget(EnE = new QLineEdit(QString::number(0.0, 'f', 3), this), 3, 1, 1, 3);
...
{
    if (NULL != window && window->isRunning())
    {
        window->stop();
        Start->setText("Start");
    }
    ...
    {
        if (NULL == window) window = new Window;
